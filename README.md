...
3. **The device table** gives each device one line, grouped under its host
   controller and its physical connector, in physical port order.
...
- Devices group under a host controller heading such as `═ 0000:00:14.0 ═`.
  Under it come the controller's bus lines, then one heading per physical
  connector that has a device on it.
- A bus line such as `▶ Bus 03 (USB2 side)  480 Mbps · 1.2% busy  rx … tx …`
  carries the bus's speed, its saturation, and the totals of every shown
  device on that bus. Only the root hub (Port `-`) and a device sysfs could
  not resolve (Port `?`) list under it. A bus at 480 Mbps or below takes
  the "USB2 side" label, a faster bus "USB3 side"; an unknown speed takes
  no label.
- A connector heading such as `▶ Port 1.4 · bus 03 + 04 · hub  rx … tx …`
  names the physical port position (root port 1, then hub port 4), the
  buses the connector spans, `hub` when the device on it has ports of its
  own, and the totals of its rows. The USB2 half and the USB3 half of one
  hub or one connector sit under the same heading: usbtop-ng follows the
  kernel's port `peer` links, which pair the two sides of a receptacle at
...
- Connectors sort by port chain, level by level. The Port column prints
  the device's own chain: `1.4.2` for a hub chain, `-` for a root hub, `?`
  when sysfs did not resolve the device.
...
1. Unplug every external hub and device you can, leaving what's built in
   (and whatever you need to type with; see
   [Adjusting the snapshot](#adjusting-the-snapshot)).
...
#### Adjusting the snapshot

On a desktop the keyboard, the mouse, or their RF receiver stay plugged in
while you take the snapshot, so they are recorded as internal like
everything else. `--snapshot-internal` lists every device it is about to
record as `port_path  vid:pid  name` before writing the file; the `S`
overlay's confirmation lists them as `port_path  vid:pid` (no name),
capped at 12 with an "… and N more" line, before you press `y`. After
recording, both surfaces print the same hint sentence. To mark one of them
external again, name its port path:

    usbtop-ng --forget-internal 3-3.1

That removes the entry (all of the named ones, or none if any name matches
nothing), prints what it forgot, and leaves the rest of the file as it
was. The file is replaced atomically, so an interrupted run leaves the
previous snapshot in place, and the file is written with owner-only
permissions (0600). The change takes effect the next time usbtop-ng
starts.

The file is plain TOML and can also be edited by hand. Each entry looks
like this; delete a block to make that device external, or add one to make
a device internal:
...
`port_path` is the sysfs name the table shows as bus and Port: bus `003`,
Port `1.4` is `3-1.4`; a root hub is `usbN`. The IDs are four lowercase hex
digits.

An entry matches only the same device on the same port, so a device you
move to another connector reads as external until you re-snapshot or edit
the entry. That is the useful default: removable gear is what you can move
when a connector or a bus turns out to be the bottleneck, and the
connector heading tells you which one it is on now.

The firmware's own claim about each port (`hardwired` or `hotplug` in
sysfs) is not used for this. It describes the port, not the device; on a
desktop a receiver in a front-panel port is removable whatever the port
says, and the snapshot is the one list you control.
...
- `cargo test --all-targets` runs the hermetic unit suite (several hundred tests)
...
Options:
  ...
  -r, --refresh <REFRESH>
      ...
      --forget-internal <PORT_PATH>
          Forget an internal-device snapshot entry by its sysfs port path (repeatable; see --snapshot-internal's listing), then exit
      --support [<PATH>]
