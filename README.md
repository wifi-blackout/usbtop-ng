...
- **Clean restore on panic and on signals**: a panic, a `SIGHUP`, a `SIGTERM` or a closing terminal emulator all leave through the same teardown as `q` — raw mode off, alternate screen left, cursor back, stdout's original flags restored. The teardown is bounded: its own writes give up after a quarter of a second, the render pipeline is switched off before the descriptor goes back to blocking, and the exit skips its usbmon question outright if the terminal would not take the restore. So nothing usbtop-ng owns waits on a terminal that has stopped reading. What is *not* bounded is what any Rust program does next — a panic's backtrace is std's own write to stderr, and stderr is nobody's to make non-blocking — so a panic into a terminal that has stopped reading still waits there, exactly as any other program's would.
