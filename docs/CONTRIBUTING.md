...
3. Run the tests:
   ...
   cargo test
   ```
   The unit suite reports 665 passed; the `tests/` directory adds the
...
├── connector/        # Physical connectors from sysfs port objects and peer links
│   └── mod.rs
...
hermetic. The unit suite reports 665 passed, working against
...
1. Run the suite with the feature:
   ...
   ```
   The unit suite reports 665 passed under the feature, the same as
