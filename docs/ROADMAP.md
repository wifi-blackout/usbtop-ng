...
2026-08-30 fleet probe found them on the two x86 hosts -- `tgl-tb4` (typec +
power-delivery + thunderbolt) and `cezanne` (typec + power-delivery, no
thunderbolt) -- and the 2026-09-03 probe of `tgl-x360` adds a third
...
feature reads that host's sysfs. `cezanne`'s USB-C/PD port is free today;
both of `tgl-tb4`'s show a connected partner, so free one before testing
...
comparison on 2026-09-01 (`devhost`, Chicony webcam, 100 raw YUYV
...
`tests/fixtures/hosts/devhost-*/stage2` bundle repeats this measurement
...
- Fixed: the usbmon text fallback's isochronous counts. The kernel prints
  ...
  stream (`tgl-tb4`, where the buffer size read 15.4x) and 1.011x on a
  continuous YUYV stream (`devhost`, buffer size 3.98x); exact whenever
...
targets exist as named, SSH-reachable hosts -- `pi-4` (Pi 4), `pi-400`
(Pi 400), `pi-5` (Pi 5), `pi-zero` (Pi Zero W, armv6l), `rock5c`
(ROCK 5C), and `bm1684x` (BM1684x board) -- alongside three x86 hosts, `tgl-tb4`
(Intel Tiger Lake, with Thunderbolt 4), `cezanne` (AMD Cezanne, USB 3.1
Gen 2), and `tgl-x360` (added 2026-09-03: Intel Tiger Lake on Kali
...
already shape the work: `rock5c` ships usbmon built in with
`/dev/usbmon0..8` live, so the binary-only vendor-kernel path is testable
today; `bm1684x`'s 5.4 vendor kernel has no usbmon module at all
...
- Build targets. 64-bit boards (Pi 4, Pi 400, Pi 5, ROCK 5C, BM1684x board on a
...
- Controller matrix. Each board exercises a different host stack: the
  ...
  Rockchip RK3588-class OTG plus xHCI mix, and the BM1684x board's SOPHGO
...
found it absent on every ARM board (present only on the x86 hosts `tgl-tb4`
and `cezanne`). `CONFIG_BPF_SYSCALL`, `CONFIG_KPROBES`, and the
...
Per host: `rock5c` (RK3588S2, arm64, 6.1) is the natural first target --
arm64 with kprobe and BPF confirmed, so it needs only the arm64 `pt_regs`
and an external BTF. `pi-5`, `pi-4`, and `pi-400` (arm64, rpt kernels)
follow the same recipe, sourcing BTF from a matching debuginfo build or a
custom kernel. `pi-zero` (armv6l, Pi Zero) is out of scope: 32-bit ARM
eBPF and CO-RE tooling are immature, and usbmon is the right backend
there. `bm1684x` (BM1684x, 5.4 vendor) is the worst case -- no usbmon and
...
throughput on every ARM board except `bm1684x`, so the eBPF backend's ARM
upside is future per-process attribution and an in-kernel second opinion
on the usbmon figures. The concrete next step is a spike on `rock5c` --
...
- Thunderbolt and USB4 hardware validation. usbtop-ng observes USB URBs
  ...
  resume, and peer links. The x86 `tgl-tb4` host (Intel Tiger Lake-LP,
...
observes. On a 7.3 kernel, repeat the `devhost` stage2 procedure in
