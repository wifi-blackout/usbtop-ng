...
- `diag/redact.rs`: the privacy rules as pure functions. Home paths become
  ...
  lines, filesystem UUIDs in the kernel command line (`/proc/cmdline` and
  the log's echo of it), and the user's connector labels in the
  preferences copy are masked (that copy is re-serialized, so its comments
  are dropped), and the
...
pub struct ControllerView {
    pub id: String,
    pub buses: Vec<BusView>,            // summary lines, bus order
    pub connectors: Vec<ConnectorView>, // chain order
}
pub struct BusView {
    pub bus_id: u8,
    pub speed: UsbSpeed,
    pub side_label: &'static str,      // "USB2 side" / "USB3 side" / ""
    pub devices: Vec<DeviceRow>,       // the root hub, unresolved devices
    pub busy_percentage: Option<f64>,
    pub rx_bps: f64, pub tx_bps: f64,  // every shown row on this bus
}
pub struct ConnectorView {
    pub key: String,      // "usb3-port1+usb4-port1", or "device:<name>"
    pub label: String,    // "Port 1.4", "Port 1 (USB3 side: 2)"
    pub buses: Vec<u8>,   // the buses the connector spans
    pub is_hub: bool,
...
**Connector pairing:** every hub, root hubs included, exposes one port
object per downstream port under its interface directory
(`usb3/3-0:1.0/usb3-port1`, `3-1/3-1:1.0/3-1-port4`; kernel
`drivers/usb/core/port.c`). A port with a companion on the controller's
other bus carries a `peer` symlink to it, and the companion links back.
`sync_from` scans those objects fresh every tick, bounded to the manager's
own devices (a hub's port objects and links appear after its device
directory does, so nothing is cached), and places each non-root device on
the connector `{its port, the port's reciprocal peer}` or, without a peer,
`{its port}`. The USB2 side (bus speed at most 480 Mbps) names the
connector; a device whose port object is absent (an older fixture, a hub
still enumerating) gets a single connector from its own chain; a device
sysfs could not resolve stays under its bus line. Retention passes
(`--filter`, hide-idle, search) prune rows first, then empty connectors,
bus lines with no row on their bus, and empty controllers, and the totals
are recomputed from what remains. A hub built into the machine on a
hardwired root port may show its two halves as two single connectors while
its downstream ports pair, because the kernel links `peer` only where
firmware pairs the ports; usbtop-ng follows the link rather than guessing.
