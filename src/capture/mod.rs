...
pub fn assert_sysfs_contained(sysfs: &Path) -> anyhow::Result<()> {
    ...
    // Reciprocal `peer` links are a two-node cycle: descend into each
    // real directory once.
...
mod tests {
    ...
    #[test]
    fn containment_check_terminates_on_a_symlink_cycle() {
        // Two directories linking to each other, as reciprocal `peer` links
        // do. A walk without a visited set never returns; run it on a thread
        // and fail if it has not answered within the deadline.
        let temp = tempfile::tempdir().unwrap();
        ...
        std::thread::spawn(move || {
            let _ = tx.send(assert_sysfs_contained(&sysfs).is_ok());
        });
        match rx.recv_timeout(std::time::Duration::from_secs(10)) {
            Ok(contained) => assert!(contained, "an in-bundle cycle is contained"),
            Err(_) => panic!("assert_sysfs_contained looped on a two-node symlink cycle"),
        }
    }
}
