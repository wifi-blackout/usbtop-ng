...
/// Every `peer` link in every committed bundle points at a port that is in
/// the same bundle and points back: the pairing the connector view follows
/// is never one-sided in the corpus.
#[test]
fn every_peer_link_in_the_corpus_is_reciprocal() {
    for sysfs in find_named(&fixtures_root(), "sysfs") {
        let index = crate::connector::PortIndex::scan(&sysfs);
        for (name, info) in index.iter() {
            let Some(peer) = &info.peer else { continue };
            let back = index.get(peer).unwrap_or_else(|| {
                panic!(
                    "{}: {name} peers with {peer}, which is not in the bundle",
                    sysfs.display()
                )
            });
            assert_eq!(
...
fn bundle_index(host: &str) -> crate::connector::PortIndex {
    let sysfs = fixtures_root().join(host).join("stage1").join("sysfs");
    assert!(sysfs.is_dir(), "{} is missing", sysfs.display());
    crate::connector::PortIndex::scan(&sysfs)
}

fn peer_of(index: &crate::connector::PortIndex, port: &str) -> Option<String> {
    index
        .get(port)
        .unwrap_or_else(|| panic!("{port} is not in the bundle"))
        .peer
        .clone()
}

/// The development desktop: root ports paired across bus 3 and bus 4, a
/// four-level hub chain paired at its deepest level, and hardwired root
/// ports on the other controller with no peer.
#[test]
fn the_development_host_bundle_pins_nested_and_root_pairings() {
    let index = bundle_index("devhost-2026-09-06");
    assert_eq!(peer_of(&index, "usb3-port1").as_deref(), Some("usb4-port1"));
    assert_eq!(peer_of(&index, "usb4-port1").as_deref(), Some("usb3-port1"));
    assert_eq!(
        peer_of(&index, "3-1.4.1.2-port3").as_deref(),
        Some("4-1.4.1.2-port3")
    );
    assert_eq!(
        peer_of(&index, "usb1-port2"),
        None,
        "a hardwired root port has no companion"
    );
    let (own, peer) = index
        .connector_of("3-1.4.1.2.3")
        .expect("the device on the deepest hub port");
    assert_eq!(own.chain, vec![1, 4, 1, 2, 3]);
    assert_eq!(
        peer.map(|p| (p.name, p.bus)),
        Some(("4-1.4.1.2-port3".to_string(), 4))
    );
    assert!(index.is_hub("3-1.4.1.2"));
}

/// The Radxa board: the USB3 root port on bus 3 pairs with bus 6, not bus
/// 4, and the USB2-only buses have unpaired ports.
#[test]
fn the_rock5c_bundle_pins_non_adjacent_bus_pairing() {
    let index = bundle_index("rock5c-2026-09-06");
    let (paired, _) = index
        .iter()
        .find(|(name, info)| name.starts_with("usb3-port") && info.peer.is_some())
        .expect("a paired root port on bus 3");
    let peer = peer_of(&index, paired).unwrap();
    assert!(peer.starts_with("usb6-port"), "{paired} pairs with {peer}");
    assert!(
        index
            .iter()
            .any(|(name, info)| name.starts_with("usb4-port") && info.peer.is_none()),
        "bus 4 is USB2-only and its ports have no companion"
    );
    // The concrete pair verified against the captured bundle.
    assert_eq!(peer_of(&index, "usb3-port1").as_deref(), Some("usb6-port1"));
    assert_eq!(peer_of(&index, "usb6-port1").as_deref(), Some("usb3-port1"));
}

/// The laptop: the two sides of one connector use different port numbers.
#[test]
fn the_tgl_x360_bundle_pins_differing_port_numbers() {
    let index = bundle_index("tgl-x360-2026-09-06");
    assert_eq!(peer_of(&index, "usb4-port2").as_deref(), Some("usb3-port1"));
...
/// Every devhost ground-truth iso bundle (see its `[generator]` note) is a
        ...
        .filter(|b| {
            b.dir.ends_with("stage2")
                && b.dir
                    ...
                    .is_some_and(|name| name.starts_with("devhost-"))
        })
        .collect::<Vec<_>>();
    assert!(
        !bundles.is_empty(),
        "the devhost ground-truth bundle is committed"
    );
    for bundle in bundles {
        let text = std::fs::read_to_string(bundle.dir.join("meta.toml")).unwrap();
        let value: toml::Value = toml::from_str(&text).unwrap();
...
///   USBTOP_NG_BLESS_BUNDLE=tgl-tb4-2026-08-31/stage2 cargo test bless_named_bundle -- --ignored --nocapture
#[test]
#[ignore]
