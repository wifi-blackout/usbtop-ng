...
struct Cli {
    ...
    /// Forget an internal-device snapshot entry by its sysfs port path
    /// (repeatable; see --snapshot-internal's listing), then exit
    #[arg(
        long,
        value_name = "PORT_PATH",
        conflicts_with_all = [
            "snapshot_internal", "once", "batch", "support", "update_usbids",
            "setup", "create_alias", "print_man", "print_completions",
            "json", "output"
        ]
    )]
    forget_internal: Vec<String>,
    ...
    #[arg(
        long,
        value_name = "DIR",
        conflicts_with_all = ["support", "snapshot_internal", "forget_internal"]
    )]
...
fn main() -> Result<()> {
    ...
    if cli.snapshot_internal {
        ...
        // the snapshot.
        let usbids = snapshot_usbids(&cli, &preferences);
        ...
        println!(
            "{}",
            snapshot::recorded_message(snapshot.devices.len(), &dest)
        );
        return Ok(());
    }

    // `--forget-internal` edits the snapshot file in place: same home
    // resolution as `--snapshot-internal`, no sysfs, no usbmon.
    if !cli.forget_internal.is_empty() {
        ...
        let Some(mut snapshot) = snapshot::Snapshot::load(&path) else {
            ...
            // (which it does not). So this names the file and both
            // possibilities, and points at no warning that may not be there.
            eprintln!("error: could not read or parse {}", path.display());
            process::exit(1);
        };
        match snapshot.forget(&cli.forget_internal) {
            Ok(removed) => {
                snapshot.write_to(&path)?;
                let usbids = snapshot_usbids(&cli, &preferences);
                for device in &removed {
                    println!("{}", snapshot::forgot_line(device, usbids.as_ref()));
                }
            }
            Err(unmatched) => {
                for port_path in unmatched {
                    eprintln!("error: no internal-device entry for {port_path}");
                }
                process::exit(1);
            }
        }
    ...
    let mut app = UsbTopApp::new(Duration::from_millis(effective_refresh_ms(cli.refresh)))
        ...
        .with_demand_basis(cli.demand.clone().into());
...
/// The usb.ids database a one-shot snapshot command names devices with:
/// the same source order the monitoring path uses (CLI flag, preferences
/// key, the home copy), with a missing HOME meaning no names rather than a
/// failure.
fn snapshot_usbids(cli: &Cli, preferences: &config::Preferences) -> Option<usbids::UsbIds> {
    let usbids_home_copy = preferences_path().ok().map(|p| p.with_file_name("usb.ids"));
    usbids::resolve_database(
        cli.usbids.as_deref().map(Path::new),
        preferences.usbids_path.as_deref().map(Path::new),
        usbids_home_copy.as_deref(),
    )
...
mod tests {
    ...
    #[cfg(feature = "capture-fixture")]
    mod capture_fixture_conflicts {
        use super::*;

        /// `--capture-fixture` runs late in `main`, after the one-shot
        /// handlers below have already returned, so pairing it with one of
        /// them would silently run the other thing and never capture. clap
        /// must reject the combination instead.
        #[test]
        fn cli_rejects_capture_fixture_with_the_one_shot_modes() {
            use clap::Parser;
            assert!(
                Cli::try_parse_from([
                    "usbtop-ng",
            ...
            assert!(
                Cli::try_parse_from(["usbtop-ng", "--capture-fixture", "d", "--support"]).is_err(),
                "--support"
            );
            assert!(
                Cli::try_parse_from(["usbtop-ng", "--capture-fixture", "d", "--snapshot-internal"])
                    .is_err(),
                "--snapshot-internal"
            );
            let ok = Cli::try_parse_from(["usbtop-ng", "--capture-fixture", "d", "--window", "5"])
                .expect("a plain capture-fixture run still parses");
            assert_eq!(ok.capture_fixture.as_deref(), Some("d"));
        }
    }
