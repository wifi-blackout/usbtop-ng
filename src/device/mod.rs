...
impl UsbDevice {
    ...
    /// The name is read by `connector::parse_device_name`, the crate's one
    /// parser of device names, so this chain and the connector label always
    /// agree.
    pub fn port_chain(&self) -> Option<Vec<u32>> {
        let name = self.sysfs_path.as_ref()?.file_name()?.to_str()?;
        crate::connector::parse_device_name(name).map(|(_, chain)| chain)
    }
...
mod tests {
    ...
    fn port_chain_parses_topology_names() {
        ...
        assert_eq!(
            mk("x-1").port_chain(),
            None,
            "a non-numeric bus is not a device name, whatever follows the dash"
        );
        assert_eq!(UsbDevice::new(3, 9).port_chain(), None); // no sysfs_path
    }

    /// The Port column (`port_chain`) and the connector label (the
    /// `connector` module's parsers) must never disagree about a name, so
    /// `port_chain` is the connector parser's chain, nothing more.
    #[test]
    fn port_chain_is_the_connector_parsers_chain() {
        let temp = tempfile::tempdir().unwrap();
        for name in [
            "usb1", "3-1", "3-1.4.2", "12-10.3", "garbage", "x-1", "3-x", "usb300", "3-1:1.0",
        ] {
            let mut d = UsbDevice::new(3, 2);
            d.sysfs_path = Some(temp.path().join(name));
            assert_eq!(
