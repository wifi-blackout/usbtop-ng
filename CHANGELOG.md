...
- Capability call-outs for a device linked below the speed it supports: the TUI marks its `!` column with 🔺, puts the reason on a line under the row, and counts the flagged devices as `findings: N` in the header; the JSON report gains a top-level `findings` list plus `capability_mbps` and `capability_source` on every device; the text report ends with a `findings` section, or `findings: none`. The capability is the device's own statement, decoded from its BOS (sysfs `bos_descriptors`, Linux 6.9 and later), with bcdUSB 3.x standing in as a 5 Gbps floor and never more where that file is absent, labelled as such wherever it shows. Each call-out names the cause the topology proves -- an empty SuperSpeed side of the connector, a USB 2 only host port, a USB 2 only port on a hub whose SuperSpeed half is up, a slower hub above it, a host port ceiling, or an upstream that permits the speed -- and says nothing where attribution is ambiguous. The kernel pairs a hub's ports by port number, which mis-pairs a hub whose two halves sit on different port numbers (a real dock does this); the engine matches such halves itself and follows the kernel's own peer link for a device's own port. The report schema stays version 1; the fields are additive. See the README's Findings section and [docs/SCRIPTING.md](docs/SCRIPTING.md#the-findings-list).
...
- Choke points, the uplinks that would choke if every device below them pushed what it can: the TUI header carries `choke: N.NNx`, the worst hub ratio on screen, the choked hub's connector heading carries its own as `choke 3.05x (1.17G asked of 384M)`, `c` toggles the basis (`(cap)` marks the capability one) and the help overlay explains both, and `--demand link|capability` (default `link`) picks the basis for `--once`/`--batch` and the one the TUI starts at; the reports' JSON gains a top-level `chokepoints` list -- the hub, its port, capacity, demand, ratio, how many devices sit below it, its three largest contributors, and the sentence -- alongside `demand_basis` and `choke_floor`, and whose text report ends with a `chokepoints:` section or `chokepoints: none`. A hub's own link is the only stage, each side counted at the practical rate the `%busy` denominators already use, and an entry is listed only at or above the breathing room of 1.25, so the hubs that sit near 1.0x stay quiet; the `capability` basis bounds every device by the capacity of the hubs above it, so nothing under a USB 2 half asks more than 480M. It is a model of the topology and nothing is measured for it: two 480M devices under one 480M hub read 2.00x whether or not they ever transfer together. The report schema stays version 1, the three fields are additive, and every committed golden in the corpus was regenerated for them. See the README's Choke points section and [docs/SCRIPTING.md](docs/SCRIPTING.md#the-chokepoints-list).
- The support bundle carries the choke points at both bases: its fixture is replayed once and written as `report.json` (the `link` basis) and `report.capability.json` (the `capability` basis, the same document with its own `demand_basis` and `chokepoints`), and `SUMMARY.txt` gains a `choke:` line counting each basis with its worst ratio.
- The support bundle sees the PCIe side of a Thunderbolt or USB4 tunnel: `inventory/pci-removable.toml` lists the PCI devices the kernel marks removable (Linux 5.16 and later, only below a port the firmware flags as externally facing, which a Thunderbolt or USB4 port is, so a tunnel's devices are those), the bridges above them and each Thunderbolt domain's host interface, with identity, driver, negotiated and maximum link speed and width (read only while the device is awake, since the kernel wakes a sleeping device to answer them), power state, ASPM states and AER counters; and `dmesg-usb.txt` now also keeps the `pcieport`, `pciehp` and `AER:` lines, every line naming a listed device's address, and the lines of the drivers seen in front of one, so a tunneled adapter's own driver messages arrive with the report even where they name no device. When no device is marked removable the bundle says so, which an old kernel and an empty Thunderbolt port both look like.
- A terminal floor of 80x24: below it the TUI shows the current size and the minimum instead of a clipped layout, keeps measuring with only `q` and `Ctrl-C` acting (a prompt or a search left open waits, unchanged), and draws the screen once the window is large enough. The help overlay is sized to its text up to the screen, scrolls with `↑`/`↓` when it does not fit (the title says so), and no longer wraps.

### Changed

- `--capture-fixture` sysfs snapshots now carry each device's `bos_descriptors` where the kernel exposes it, reduced to its SuperSpeed and SuperSpeedPlus capability descriptors under a rebuilt header (the Container ID, a per-unit UUID, and everything else are dropped), the one binary attribute a bundle carries by design, so a replay decides capability the way the live tool does; every committed golden was regenerated for the added report fields, and the `tgl-tb4-2026-09-12` dock bundle was recaptured in place so the corpus holds real BOS bytes and pins its two expected findings. A bundle captured on an older kernel carries none and replays through the bcdUSB fallback, as before.
