...
- Degraded-terminal robustness chassis (`src/tui/`), so a terminal that cannot keep up degrades the display instead of the session:
  ...
  - Terminal restore on panic, `SIGHUP`, `SIGINT` and `SIGTERM`: signals arrive as ordinary UI events and leave through the same teardown as `q`. The restore is idempotent and bounded three ways — its own writes give up after 250ms, it trips a latch that stops the output stage writing before it hands the descriptor back to blocking, and the post-session usbmon question is skipped when the terminal would not take the restore. Nothing usbtop-ng writes can therefore be held up by a terminal that has stopped reading; a panic's backtrace still can, because that is std's write to stderr and not usbtop-ng's to bound
