...
- The support bundle sees the PCIe side of a Thunderbolt or USB4 tunnel: `inventory/pci-removable.toml` lists the PCI devices the kernel marks removable (Linux 5.16 and later; a tunnel's devices are those), the bridges above them and each Thunderbolt domain's host interface, with identity, driver, negotiated and maximum link speed and width, power state, ASPM states and AER counters; and `dmesg-usb.txt` now also keeps the `pcieport`, `pciehp` and `AER:` lines and every line naming a removable device's address, so a tunneled adapter's own driver messages arrive with the report. A kernel without the `removable` attribute is noted rather than read as "nothing removable".
